...
def pyvid_multiclass_x3d(num_classes, resize, **kwargs):
    """Constructs a multiclass single video X3D model from pytorchvideo."""
    model = ptx3d.create_x3d(
        input_clip_length=64,
...
def x3d(num_classes, **kwargs):
    """Constructs an X3D model."""
    model = X3D(Bottleneck, get_blocks("M"), get_inplanes("M"), n_classes=num_classes, **kwargs)
    return model
