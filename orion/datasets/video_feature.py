...
class Video_feature(torch.utils.data.Dataset):
    """
    ---------------------------------------------------------------------------------
    Purpose:
    ...
    def __init__(
        ...
        self.fnames, self.outcome, self.ages, self.segments = [], [], [], []
        if split == "external_test":
            ...
            with open(os.path.join(self.folder, self.filename)) as f:
                ...
                for line in f:
                    ...
                    age = lineSplit[ageIndex]
                    segment = lineSplit[segmentIndex]
                    # pdb.set_trace()
        ...
        if self.weighted_sampling == True:
            ...
            for i in range(len(w_list)):
                w_list[i] = w_max / w_list[i]
            self.weight_list = torch.tensor(w_list)
            ####
            """
    ...
    def __getitem__(self, index):
        ...
        center = int(video.shape[1] / 2)
        return (
            video[:, center - 12 : center + 12],
            torch.tensor([int(self.ages[index])]),
            torch.tensor([int(self.segments[index])]),
            target,
        )
        ###
        # return video, target
